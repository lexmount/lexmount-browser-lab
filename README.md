...
## ⭐ H100 GRPO 训练复现(最重要的入口)

想在 H100 上复现我们已验证的 Browser-RL 训练(Qwen3-8B + verl GRPO + Lexmount
云浏览器 + WebVoyager 任务,60 步 reward 均值 ~0.105 → ~0.289),只需要
`training/h100/` 这一个子目录:它自带全部启动脚本、训练运行时和任务数据,
从克隆仓库到启动训练一共六步、每步一条命令。

**启动步骤全部写在 [training/h100/README.md](training/h100/README.md)**——
准备一台 8×H100、两个 API key(Lexmount + 裁判接口),照着做即可;
移植细节与差异说明见 [training/h100/PORTING.md](training/h100/PORTING.md)。
