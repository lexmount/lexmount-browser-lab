...
`task_sets/direct64-lexmount-transport5.txt` 是从 `stratified64` 直连诊断中抽出的五个
Lexmount browser/CDP transport 失败任务。它在 `c=1` 下采用两次反向顺序重复，用于区分并发/会话
生命周期问题与单站点波动；不用于重新估计 LexBench 总体成功率。
