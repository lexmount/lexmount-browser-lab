...
def initialize_global_process_group_ray(timeout_second=None, backend=None):
    ...
    # PyTorch's implicit Gloo timeout is 30 minutes. Large sharded FSDP
    # checkpoints written through shared storage can legitimately exceed that,
    # so use an explicit, operator-controlled timeout for Ray worker groups.
    if timeout_second is None:
        timeout_second = int(os.environ.get("VERL_PROCESS_GROUP_TIMEOUT_SECONDS", "7200"))
    if timeout_second <= 0:
        raise ValueError("VERL_PROCESS_GROUP_TIMEOUT_SECONDS must be positive")
    timeout = timedelta(seconds=timeout_second)
