...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # LexBrowser patch: forward the agent loop's explicit
        # environment-invalid flags to the estimator so group statistics are
        # computed over valid samples only.  An all-zero response_mask is not
        # sufficient evidence of environment failure (a policy that emits an
        # immediate stop token also produces a zero-token response), so the
        # flag — not the mask — carries the exclusion semantics.  When the
        # flags are absent (e.g. a non-LexBrowser dataset), env_invalid stays
        # None and the estimator falls back to its mask heuristic.
        env_invalid = None
        invalid_flags = data.non_tensor_batch.get("lexbrowser_invalid_sample")
        if invalid_flags is not None:
            env_invalid = torch.tensor(
                [bool(flag) for flag in invalid_flags],
                dtype=torch.bool,
