...
REFERENCE_GROUNDED_TASK_JUDGE_PROMPT = """You are evaluating whether a browser automation agent successfully completed a web task.

Judge the agent's actual final answer together with the recorded browser evidence. Do not give credit for an unsupported claim, and do not give credit merely because the browser visited a relevant page.

Reference-answer policy:
- A `golden` reference is a strong semantic constraint. A conflicting final answer should be rejected unless the browser evidence clearly proves that the live website has changed.
- A `possible` reference is illustrative and may be non-exhaustive or time-sensitive. Accept a different answer when it satisfies every task requirement and is supported by current browser evidence.
- The site notice describes known sources of live-data drift. The browser evidence is authoritative for current prices, dates, availability, rankings, and other changing facts.

If the task requests information and the final answer is empty, incomplete, or omits a required field, respond "no". If the task requests a browser-side action, require the transcript to show that the requested end state was reached.
If the transcript is truncated and the remaining evidence does not clearly prove success, respond "no".
...
Reference Answer Type: {reference_type}
...
class LexmountCDPSession:
    ...
    def __init__(
        ...
        timeout_s: float,
        *,
        isolated_browser_context: bool = False,
    ) -> None:
        ...
        if isolated_browser_context:
            self._browser_context_id = self.call("Target.createBrowserContext")[
                "browserContextId"
            ]
            target_id = self.call(
                "Target.createTarget",
            ...
            if page is None:
                target_id = self.call("Target.createTarget", {"url": "about:blank"})[
                    "targetId"
                ]
            else:
...
class LexmountDOMMode:
    ...
    async def teardown(self) -> None:
        ...
        # the service alive until every such call returns and its session is
        # closed, otherwise a graceful container restart can orphan browsers.
        pending_cleanups = tuple(self._background_session_cleanup_tasks)
        if pending_cleanups:
            self.logger.info(
                "Waiting for %d late Lexmount session cleanup task(s)",
...
class LocalCDPMode(LexmountDOMMode):
    """Use one persistent local Chromium with an isolated context per rollout."""
    ...
    def __init__(
        ...
        *,
        ...
        self.dom_backend = "cdp"
        self.proxy_model_to_stagehand = False
        self.setup_navigation_timeout_s = setup_navigation_timeout_s
        self.per_tool_timeout_s = per_tool_timeout_s
        self.episode_timeout_s = episode_timeout_s
        self.max_repeated_tool_calls = max_repeated_tool_calls
    ...
    def _browser_websocket_url(self) -> str:
        with urllib.request.urlopen(
            f"{self.cdp_http_url}/json/version",
            timeout=min(10.0, self.setup_navigation_timeout_s),
        ) as response:
            payload = json.load(response)
        cdp_url = str(payload.get("webSocketDebuggerUrl") or "")
        if not cdp_url:
            raise RuntimeError("Local Chromium did not expose webSocketDebuggerUrl")
        return cdp_url
    ...
    async def setup_state(self, state: vf.State) -> vf.State:
        ...
        state["local_cdp_slot_acquired"] = True
        create_started_at = time.monotonic()
        try:
            cdp_url = await asyncio.to_thread(self._browser_websocket_url)
            state["browser_session"] = await asyncio.to_thread(
                LexmountCDPSession,
                cdp_url,
                self.setup_navigation_timeout_s,
                isolated_browser_context=True,
            )
            guard.record_timing(
                "local_cdp_context_create_seconds",
                create_started_at,
                phase="local_cdp_context_create",
            )
            return state
        except Exception as exc:
            guard.record_timing(
            ...
            self._release_local_slot(state)
            raise

    def _release_local_slot(self, state: vf.State) -> None:
        if state.pop("local_cdp_slot_acquired", False):
            self._slots.release()

    async def cleanup_session(self, state: vf.State) -> None:
        try:
            browser_session = state.pop("browser_session", None)
            if isinstance(browser_session, LexmountCDPSession):
                await asyncio.to_thread(browser_session.close)
        finally:
            self._release_local_slot(state)
