...
LEXBROWSER_DYNAMIC_SAMPLING=${LEXBROWSER_DYNAMIC_SAMPLING:-0}
LEXBROWSER_GROUP_RESAMPLES=${LEXBROWSER_GROUP_RESAMPLES:-2}
...
docker run -d --name "$NAME" --network host --ipc host --shm-size "$SHM_SIZE" \
  ...
  -e LEXBROWSER_DYNAMIC_SAMPLING="$LEXBROWSER_DYNAMIC_SAMPLING" \
  -e LEXBROWSER_GROUP_RESAMPLES="$LEXBROWSER_GROUP_RESAMPLES" \
