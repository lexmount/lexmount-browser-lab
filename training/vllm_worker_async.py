...
# Stagehand parses the *assistant content* from its private DOM-helper request
# directly as JSON.  Qwen3's stock template uses an empty <think> preamble
# even when ``enable_thinking=False``; that protocol is valid for the policy
# path but not for Stagehand's JSON.parse().  This template is used only for
...
class VllmAsyncGenerationWorker(BaseVllmGenerationWorker):
    ...
    def _setup_vllm_openai_api_server(self, app: FastAPI) -> FastAPI:
        ...
        serving_chat_default_kwargs = dict(
            ...
            # This endpoint is only reachable through the colocated
            # NeMo-Gym model proxy. Permit the Stagehand-only structured
            # helper template selected below; policy requests never provide
            # a request-level template.
        ...
        # Stagehand's JSON helper shares the engine/weights with policy
        # rollout, but must not run the policy's reasoning/tool output
        # parsers. Those parsers are correct for on-policy Hermes calls and
        # wrong for a response_format JSON consumer.
        stagehand_serving_chat_kwargs = serving_chat_kwargs | {
            "reasoning_parser": "",
            "tool_parser": None,
            "enable_auto_tools": False,
        }
        openai_serving_chat_stagehand = NeMoRLOpenAIServingChat(
            **stagehand_serving_chat_kwargs
        )
        ...
        async def create_chat_completion(
            ...
            # NeMo Gym policy calls carry token-prefix metadata and must remain
            # exactly on-policy. Stagehand also uses this endpoint for DOM
            # planning, but those environment-side helper calls intentionally
            # use their own sampling parameters (usually lower temperature).
            # The first policy turn necessarily has no prefix tokens.  NeMo
            # Gym's policy client therefore sets an explicit private header;
            # later turns also carry the prefix metadata.  Stagehand does not
            # set this header, so its DOM helper requests are the only ones
            # that receive the strict no-thinking JSON template.
            # Keep this compact diagnostic until the two request shapes are
            # proven distinct in a real Stagehand rollout.  It deliberately
            # logs only protocol metadata (never messages, URLs, or keys).
            # Stagehand's DOM helpers can themselves use OpenAI ``tools`` for
            # schema enforcement, so ``tools`` alone cannot be a durable
            # policy/helper discriminator.
            request_has_response_format = getattr(request, "response_format", None) is not None
            tool_names = []
            for tool in request.tools or []:
                if isinstance(tool, dict):
                    ...
                    tool_names.append(str(getattr(function, "name", None) or getattr(tool, "name", None) or "?"))
            print(
                "[LEX_REQUEST_CLASSIFY] "
                f"policy_header={raw_request.headers.get('X-Nemo-Policy-Rollout') == '1'} "
                f"prefix={request.required_prefix_token_ids is not None} "
                f"tools={bool(request.tools)} response_format={request_has_response_format} "
                f"tool_names={tool_names[:8]} model={request.model!r}",
                flush=True,
            )
            is_stagehand_structured_helper = (
                request_has_response_format
                and request.required_prefix_token_ids is None
                and not bool(request.tools)
            )
            is_policy_request = (
                raw_request.headers.get("X-Nemo-Policy-Rollout") == "1"
                or request.required_prefix_token_ids is not None
                # The NeMo-Gym proxy does not forward arbitrary HTTP headers
                # to this internal vLLM endpoint.  Browser policy requests do
                # carry the registered browser function definitions, whereas
                # Stagehand observe/act/extract helper calls request a strict
                # JSON object and do not carry policy tools.  This also marks
                # the first policy turn, which has no prefix yet.
            ...
            if is_stagehand_structured_helper:
                # Template selection happens before tokenization/generation;
                # no post-hoc text rewriting touches the policy trajectory.
                request.chat_template = STAGEHAND_STRUCTURED_CHAT_TEMPLATE
            ...
            serving_chat = (
                openai_serving_chat_stagehand
                if is_stagehand_structured_helper
                else openai_serving_chat
            )
            generator = await serving_chat.create_chat_completion(
                request, raw_request
            )

            if isinstance(generator, ErrorResponse):
                print(
                    f"[LEX_CHAT_ERROR] helper={is_stagehand_structured_helper} "
                    f"error={generator.model_dump()}",
                    flush=True,
                )
                return JSONResponse(
                ...
                # vLLM's ``abort`` is not an OpenAI Chat Completions terminal
                # enum. NeMo-Gym validates this HTTP response before the
                # agent client can normalize it, so adapt it at the server
                # boundary without touching generated text or token IDs.
